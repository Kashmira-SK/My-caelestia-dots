...
#### Central Dynamic theme routing

- Continued centralizing wallpaper-driven theme behavior through `dynamic-theme/apply_theme.py`.
- Dynamic wallpaper generation, previews, light/dark mode changes, Material variant changes, and scheme-picker previews now share the same generator path.
- `~/.local/state/caelestia/scheme.json` remains the canonical active palette state.
- Static schemes continue to use Caelestia's normal static scheme implementation.

#### Light-mode surface tinting

Added wallpaper-derived tinting for Dynamic light mode.

Previously, Material light mode produced very pale neutral surfaces that caused the Quickshell UI to appear almost completely white with only small accent elements retaining wallpaper colour.

Added helpers in `dynamic-theme/apply_theme.py` to blend light surface roles toward the generated `primaryContainer`.

Affected Material roles include:
...
Semantic foreground roles remain generated by Material instead of being manually recoloured.

Examples of roles intentionally left under Material contrast handling include:
...
Light themes now retain a stronger pastel version of the wallpaper colour family instead of reading as plain white.

#### Persistent Dynamic profiles

Added three wallpaper-driven Dynamic profiles:
...
Profile state is stored separately from the Material variant.

Added persistent state file:

```text
~/.local/state/caelestia/dynamic-profile.txt
```

Added profile state helpers:
...
The selected profile survives subsequent wallpaper applications.

For example:

```bash
python3 ~/.config/caelestia/apply_theme.py --profile vivid
python3 ~/.config/caelestia/apply_theme.py
```

The second command continues using the selected `vivid` profile.
...
`--current-selection` reports the currently active scheme/profile/variant combination.
...
#### Profile and Material variant separation

Dynamic profiles are independent from Material variants.

Profiles no longer intentionally replace or select the Material variant.

The theme system can therefore represent combinations such as:
...
Changing the profile preserves the currently selected Material variant.

Verified with:

```bash
python3 ~/.config/caelestia/apply_theme.py --variant expressive
python3 ~/.config/caelestia/apply_theme.py --profile vivid
python3 ~/.config/caelestia/apply_theme.py --current-selection
```
...
#### Dynamic profile palette processing

`baseline` keeps the generated Material palette unchanged apart from the existing Dynamic light-surface processing.

`vivid` and `muted` add profile-level processing on top of the selected Material variant.

The profile processing targets accent-related roles rather than intentionally replacing the neutral background/surface family.

Accent roles currently involved include:
...
The Vivid profile currently uses a stronger accent source while preserving the selected Material variant as the actual active variant.

The Muted profile reduces accent chroma while retaining the wallpaper hue family.

The profile architecture is complete, but the exact visual strength of `vivid` and `muted` still needs further tuning.

#### `--list-schemes`

Updated `list_schemes()` so Dynamic profile previews are generated centrally through `generate_dynamic_scheme()`.

Dynamic picker data now contains:
...
```bash
python3 ~/.config/caelestia/apply_theme.py --list-schemes \
  | jq '.dynamic | keys'
```
...
Scheme-list generation remains side-effect-free and does not apply a theme or overwrite the active `scheme.json`.

---

### `services/Colours.qml`

Updated Dynamic scheme selection so the scheme flavour is treated as the Dynamic profile.

Dynamic selections now call:

```text
apply_theme.py --profile <profile>
```

instead of simply invoking `apply_theme.py` without identifying the selected profile.

This allows the shared scheme UI to represent:
...
while still using the existing:
...
Existing Dynamic mode routing remains centralized through:

```qml
Colours.setMode(mode)
```

Existing Material variant routing remains centralized through:
...
Updated the launcher scheme service to use the new Dynamic profile model.

The visible scheme list was reduced from the previous filtered static-theme list to Dynamic profiles.

Current Dynamic picker entries are:
...
The picker continues to obtain palette previews from:

```bash
python3 ~/.config/caelestia/apply_theme.py --list-schemes
```

Changed current-selection lookup from:

```bash
caelestia scheme get -nfv
```

to the centralized Dynamic-aware command:

```bash
python3 ~/.config/caelestia/apply_theme.py --current-selection
```

This allows the picker to correctly distinguish:
...
even though all three use the same underlying Caelestia scheme name.

The service continues to expose:
...
for launcher and Control Center selection state.

---

### `modules/controlcenter/appearance/sections/ColorSchemeSection.qml`

No new profile-specific routing was required.

The Control Center already consumes:
...
Therefore the new Dynamic profile entries automatically use the same model and selection path as the launcher.

---

### `services/Wallpapers.qml`

Fixed wallpaper preview state handling.

Previously, `Colours.showPreview` could remain enabled after selecting a wallpaper.

The problematic lifecycle was:

```text
wallpaper preview starts
...
launcher closes
→ stopPreview()
→ preview palette intentionally stays active while wallpaper applies

wallpaper path updates
→ previewColourLock becomes false
→ Colours.showPreview could remain true
```

This allowed Quickshell to continue displaying stale preview colours even though the real active `scheme.json` had already changed.

Updated the wallpaper-state handling so the preview colour lock is released when the newly selected wallpaper becomes active and the UI returns to the real palette.

Also guarded the asynchronous preview result so an old preview process cannot reactivate preview colours after preview mode has already ended.

This fixed a case where:
...
could correctly change between light/dark while Quickshell itself remained visually stuck on an older preview palette.

---

### `dynamic-theme/patches/__init__.py`

Removed the legacy tracked Material override:

```text
dynamic-theme/patches/__init__.py
```

Dynamic generation no longer requires replacing Caelestia's top-level Material helper module.

---

### `/usr/lib/python3.14/site-packages/caelestia/utils/material/__init__.py`

Restored the stock Caelestia CLI `v1.0.7` Material module.

The restored module provides the original:

```python
get_score_for_image()
get_colours_for_image()
```

implementation.

Verified that the stock module imports and generates colours successfully.

The old custom Material override is therefore no longer required for normal Dynamic operation.

---

### `dynamic-theme/patches/wallpaper.py`

Kept the wallpaper compatibility patch.

This file is still required because the current wallpaper command path still passes through Caelestia:

```text
Quickshell
→ caelestia wallpaper -f
→ patched wallpaper.py
→ wallpaper state update
→ wallpaper postHook
→ apply_theme.sh
→ apply_theme.py
```

The patch continues to prevent Caelestia's built-in Dynamic generation from competing with the central engine.

It also keeps CLI wallpaper previews routed through the same central preview implementation.

Verified:

```bash
caelestia wallpaper -p "$wall"
```

still returns the central Dynamic preview palette after restoring the stock Material module.
...
The active Qt configuration remains intentionally based on:
...
No functional qt6ct integration was removed.

---

### `dynamic-theme/caelestia-theme-watch.fish`

No removal was made to the Qt compatibility path.

The watcher intentionally continues generating:
...
because the current desktop still uses `qt6ct`.

The watcher also continues handling:

```text
startpage updates
Starship runtime colour generation
```
...
### Dynamic theme verification

Verified the central Dynamic engine after restoring Caelestia's stock Material helper.

Stock Caelestia Material path:

```text
stock material ok
```

Central Dynamic preview:

```text
central preview ok
```

Patched wallpaper CLI preview:

```text
cli preview ok
```

Verified Dynamic profile persistence:

```text
select vivid
→ apply theme
→ apply theme again without --profile
→ current selection remains vivid
```
...
Verified profile/variant independence:
...
instead of changing the Material variant.

---

### Current Dynamic theme architecture
...
dynamic-theme/patches/wallpaper.py
    │
    ├── updates wallpaper state
    ├── avoids duplicate built-in Dynamic generation
    └── executes wallpaper postHook
    │
    ▼
dynamic-theme/apply_theme.sh
    │
    ▼
dynamic-theme/apply_theme.py
    │
...
Completed non-documentation cleanup:

```text
central Dynamic generation
central Dynamic previews
central Dynamic mode routing
central Material variant routing
wallpaper preview routing
wallpaper preview-state fix
light-mode wallpaper tinting
persistent Dynamic profiles
profile / variant separation
stock Caelestia Material module restoration
legacy Material patch removal
qt6ct environment duplicate cleanup
```

Intentionally retained compatibility components:
...
These remain necessary for the current wallpaper and qt6ct integration.

Remaining theme work:

```text
fine-tune Vivid accent strength
fine-tune Muted accent strength
